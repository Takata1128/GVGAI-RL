...
class DQN():
    def __init__(self, env, config: DQNConfig):
        ...
        # ネットワーク
        self.net = QNetwork(env.observation_space.shape,
                            n_action=env.action_space.n).to(self.device)
        self.target_net = QNetwork(
            env.observation_space.shape, n_action=env.action_space.n).to(self.device)
        ...
        self.epsilon_func = lambda step: max(
            epsilon_end,
            epsilon_begin - (epsilon_begin - epsilon_end) *
            (step / epsilon_decay),
        )
    ...
    def train(self):
        ...
        with wandb.init(project=self.config.project, config=self.config.__dict__):
            for step in range(1, self.n_steps+1):
                # 1ステップインタラクション
                action = self.net.act(obs.float().to(
                    self.device), self.epsilon_func(step))
                ...
                # Qネットワーク更新
                if len(self.replay_buffer) > self.seed_buffer_size and step % self.train_every == 0:
                    ...
                    if len(scores) > 100:
                        ...
                        if current_average > best_mean_score:
                            ...
                            result_dir = os.path.join(
                                ...
                                "{}_{}".format(self.config.env,
                                               self.config.id),
                            )
                            model_dir = os.path.join(result_dir, "dqn_models")
                            save_path = os.path.join(
                                model_dir, "models_best.pth")
